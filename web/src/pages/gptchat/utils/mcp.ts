 ...
 */
async function fetchJSONOrSSE(resp: Response): Promise<any> {
 ...
 */
function normalizeMCPToolListResponse(data: any): any[] | null {
    if (!data) return null
...
function normalizeToolsToOpenAIFormat(tools: any[]): McpTool[] {
    return tools.map(t => ({
        name: t.name,
        description: t.description,
        input_schema: t.inputSchema || t.parameters, // MCP uses inputSchema, OpenAI uses parameters
    }))
}

async function ensureMCPSession(server: any, endpointURL: string, headers: any) {
...
export async function syncMCPServerTools(server: McpServerConfig): Promise<{ updatedServer: McpServerConfig, count: number }> {
    ...
    const sessionServer = { ...server } as any
