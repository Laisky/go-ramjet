...
func runTask() {
	...
	log.Logger.Info("start backup elasticsearch logs...")
	LoadSettings() // reload
	...
	for _, st := range LoadSettings() { //reload settings
		...
		case "bos":
			loader = &bosUploader{}
		default:
			log.Logger.Error("got unknown upload mode", zap.String("mode", st.Mode))
			continue
		}
		err = loader.New(st)
		if err != nil {
			log.Logger.Error("construct uploader error", zap.Error(err))
			continue
		}
