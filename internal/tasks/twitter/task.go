...
func bindTask() {
	...
	go store.TaskStore.TickerAfterRun(gutils.Settings.GetDuration("tasks.twitter.search.sync.interval")*time.Second, syncSearch)
	go store.TaskStore.TickerAfterRun(gutils.Settings.GetDuration("tasks.twitter.search.sync.interval")*time.Second, syncReplica)
}
