 ...
 * @type {string[]}
 */
const ToolEventPrefixes = ['Upstream tool_call:', 'args:', 'exec MCP tool:', 'tool ok', 'tool error:', 'tool loop limit reached'];
...
const ChatModelKimiK2 = 'moonshotai/kimi-k2-instruct-0905';
...
const QaModels = [QAModelBasebit, QAModelSecurity, QAModelImmigrate, QAModelCustom, QAModelShared];
const ImageModels = [
  ImageModelDalle3,
  ImageModelGptImage1,
  ImageModelSdxlTurbo,
...
const CompletionModels = [CompletionModelDavinci3];
const FreeModels = [
  // ChatModelGroqLlama2With70B4K,
  // ChatModelGroqMixtral8x7B32K,
  // ChatModelGroqGemma2With9B,
...
AllModels.push(ChatModelGPTOSS120B, ChatModelGPTOSS20B);
...
let deleteCheckCallback,
   ...
   */
  deleteCheckModal;
let singleInputCallback, singleInputModal, editImageModalCallback, systemPromptModalCallback;
...
function getLiveAiRespElement(chatID) {
  ...
  const fallback = chatContainer?.querySelector(`.chatManager .conservations .chats #${chatID} .ai-response`);
...
function ensureStopGeneratingButton() {
  ...
  chatPromptInputBtn.insertAdjacentHTML(
    'beforebegin',
...
const UrlConfigBooleanFields = new Set(['all_in_one', 'disable_https_crawler', 'enable_talk', 'enable_mcp']);
const UrlConfigIntegerFields = new Set(['max_tokens', 'n_contexts', 'draw_n_images']);
const UrlConfigFloatFields = new Set(['temperature', 'presence_penalty', 'frequency_penalty']);
...
async function inpaintingImageByFlux(chatID, prompt, rawImgBlob, maskBlob) {
  globalAIRespEle = chatContainer.querySelector(`.chatManager .conservations .chats #${chatID} .ai-response`);
...
async function dataMigrate() {
  ...
  // move config from localstorage to session config
  {
    ...
    await Promise.all(
      Object.keys(storageVals).map(async (oldKey) => {
  ...
  await Promise.all(
    (await libs.KvList()).map(async (key) => {
...
function setupSingleInputModal() {
  ...
  document
    .getElementById('singleInputModal')
...
async function setupModals() {
  ...
  document
    .getElementById('deleteCheckModal')
    .querySelector('.modal-body .yes')
    .addEventListener('click', async (e) => {
      e.preventDefault();
  ...
  saveSystemPromptModelEle.querySelector('.btn.save').addEventListener('click', async (evt) => {
    evt.preventDefault();
  ...
  editImageModalEle.querySelector('.btn.save').addEventListener('click', async (evt) => {
    evt.preventDefault();
...
async function setupByUserInfo(userInfo) {
  ...
  document
    .querySelectorAll('#headerbar .chat-models li a, ' + '#headerbar .qa-models li a, ' + '#headerbar .image-models li a')
    .forEach((elem) => {
  ...
  // listen click events
  const modelElems = document.querySelectorAll(
    '#headerbar .chat-models li a, ' + '#headerbar .qa-models li a, ' + '#headerbar .image-models li a'
  );
  modelElems.forEach((elem) => {
    elem.addEventListener('click', async (evt) => {
      ...
      document.querySelectorAll('#headerbar .navbar-nav a.dropdown-toggle').forEach((elem) => {
        elem.classList.remove('active');
      });
...
function showalert(type, msg) {
  ...
  chatContainer.querySelector('.chatManager .alerts-container .alerts').insertAdjacentHTML('afterbegin', alertEle);
}
...
async function changeSession(activeSid) {
  if (globalAIRespSSE) {
    // auto stop previous sse when switch session
  ...
  try {
    const elements =
      chatContainer.querySelectorAll(
        `.role-ai .ai-response[data-task-type="${ChatTaskTypeImage}"][data-task-status="${ChatTaskStatusWaiting}"]`
      ) || [];

    await Promise.all(
      Array.from(elements).map(async (item) => {
...
async function clearSessionHandler(evt, sessionID) {
  ...
  if (!sessionID) {
    // remove all sessions and their chats
    ...
    // only remove one session's chat history, keep its config
...
function bindSessionEditBtn() {
  document.querySelectorAll('#sessionManager .sessions .session .bi.bi-pencil-square').forEach((item) => {
    ...
    item.addEventListener('click', async (evt) => {
      ...
      SingleInputModal(
        ...
        async (newSessionName) => {
          ...
          // update session name
          document.querySelector(`#sessionManager .sessions [data-session="${sid}"]`).dataset.name = newSessionName;
          document.querySelector(`#sessionManager .sessions [data-session="${sid}"] .col`).innerHTML = newSessionName;
          chatContainer.querySelector(`.sessions [data-session="${sid}"] .col`).innerHTML = newSessionName;
        },
        oldSessionName
      );
    });
  ...
  btns.forEach((item) => {
    ...
    item.addEventListener('click', async (evt) => {
      ...
      ConfirmModal(`Are you sure to delete session "${deleteSessionName}"?`, async () => {
        ...
        document.querySelectorAll(`#sessionManager .sessions [data-session="${deleteSid}"]`).forEach((item) => {
          item.remove();
        });
        chatContainer.querySelectorAll(`.sessions [data-session="${deleteSid}"]`).forEach((item) => {
          item.remove();
        });
...
async function setupSessionManager() {
  ...
  {
    document.querySelector('#sessionManager .btn.purge').addEventListener('click', clearSessionHandler);
  }
  ...
  // restore all sessions from storage
  {
    ...
    await Promise.all(
      allSessionKeys.map(async (key) => {
        ...
        document.querySelector('#sessionManager .sessions').insertAdjacentHTML(
          'beforeend',
        ...
        chatContainer.querySelector('.sessions .session-tabs').insertAdjacentHTML(
          'beforeend',
          `<div class="list-group session" data-session="${sessionID}" data-name="${sessionName}">
                        <button type="button" class="list-group-item list-group-item-action ${active}" aria-current="true">
                            <div class="col">${sessionName}</div>
  ...
  {
    document.querySelector('#chatContainer .chatManager .card-footer .scroll-down').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      scrollChatToDown();
    });
  }
  ...
  // add new session
  {
    document.querySelector('#sessionManager .btn.new-session').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      // const evtTarget = libs.evtTarget(evt);
      ...
      chatContainer.querySelector('.chatManager .conservations .chats').innerHTML = '';
      const newSessionID = maxSessionID + 1;
      document.querySelector('#sessionManager .sessions').insertAdjacentHTML(
        'beforeend',
...
function scrollChatToTop(evt) {
  ...
  chatContainer.querySelector('.chatManager .conservations').scrollTo({
    top: 0,
    left: 0,
    behavior: 'smooth',
  });
...
async function restorePinnedMaterials() {
  ...
  document.querySelectorAll('#chatContainer .pinned-refs p .bi-trash').forEach((item) => {
    item.addEventListener('click', async (evt) => {
      evt.stopPropagation();
      const evtTarget = libs.evtTarget(evt);
...
function getPinnedMaterials() {
  const urls = [];
  document.querySelectorAll('#chatContainer .pinned-refs a').forEach((item) => {
                ...
                messages: [
                  {
                    role: RoleSystem,
                    content:
                      'Please determine the user\'s intent based on the prompt. Return "image" if the user wants to generate an image, otherwise return "text". Do not provide any other irrelevant content except "image"/"text".',
                  },
                  {
                    role: RoleHuman,
                    content: prompt,
...
async function appendImg2UserInput(chatID, imgDataBase64, imgName) {
  ...
  const text = chatContainer.querySelector(`.chatManager .conservations .chats #${chatID} .role-human .text-start pre`).innerHTML;
  await saveChats2Storage({
    role: RoleHuman,
    chatID,
    content: text,
  ...
  chatContainer
    ...
    .insertAdjacentHTML('beforeend', `<img src="data:image/png;base64,${imgDataBase64}" data-name="${imgName}">`);
}
...
async function sendChat2Server(chatID, reqPrompt) {
  ...
  let selectedModel = await OpenaiSelectedModel();
  if (!chatID) {
    // if chatID is empty, it's a new request
    chatID = newChatID();
    ...
    // if chatID is not empty, it's a reload request
    reqPrompt = chatContainer.querySelector(`.chatManager .conservations .chats #${chatID} .role-human .text-start pre`).innerHTML;
  }
  ...
  switch (promptType) {
    case 'chat': {
      ...
      if (VisionModels.includes(selectedModel) && chatVisionSelectedFileStore.length > 0) {
        ...
        const userChatDisplayElement = chatContainer.querySelector(`.chatManager .conservations .chats #${chatID} .role-human .text-start`);
    ...
    case 'qa':
      ...
      switch (selectedModel) {
        ...
        case QAModelShared:
          ...
          url =
            `/ramjet/gptchat/ctx/share?uid=${urlParams.get('uid')}` +
  ...
  const handleToolCallsAndContinue = async () => {
    ...
    try {
      ...
      const followMessages = [].concat(baseChatMessagesForToolCalls || [], [assistantToolCallsMessage], toolResultMessages);
      ...
      try {
        ...
        if (annotations && annotations.length > 0) {
          ...
          annotations.forEach((annotation) => {
            ...
            if (
              !globalAIRespData.annotations.some((a) => a.type === annotation.type && a.url_citation?.url === annotation.url_citation?.url)
            ) {
        ...
        if (reasoningChunk || respChunk) {
          ...
          switch (taskStatus) {
            case 'waiting': {
              ...
              if (
                (globalAIRespData.reasoningThinkingContent && globalAIRespData.reasoningThinkingContent.trim() !== '') ||
                ...
                if (
                  toolsContainer &&
                  Array.isArray(globalAIRespData.reasoningToolEvents) &&
                  globalAIRespData.reasoningToolEvents.length > 0
                ) {
...
async function renderAfterAiResp(chatData, saveStorage = false, rootElement = null) {
  ...
  try {
    ...
    // First ensure the info div exists (idempotent)
    if (!aiRespEle.querySelector(':scope > div.info')) {
    ...
    if (sconfig.api_token.startsWith('laisky-') || sconfig.api_token.startsWith('sk-') || sconfig.api_token.startsWith('FREETIER-')) {
      // Display existing cost if we already have it
      if (costUsd) {
        if (!infoDiv.querySelector('.cost')) {
        ...
        try {
          ...
          let costPlaceholder = infoDiv.querySelector('.cost.cost-loading');
          if (!costPlaceholder) {
            costPlaceholder = document.createElement('i');
...
function bindImageOperationInAiResp(chatID) {
  const aiRespEle = chatContainer.querySelector(`.chatManager .conservations .chats #${chatID} .ai-response`);
  if (!aiRespEle) {
...
async function addOperateBtnBelowAiResponse(chatID) {
  ...
  divContainer.querySelector('.btn[data-fn="voice"]').addEventListener('click', async (evt) => {
    evt.stopPropagation();
  ...
  divContainer.querySelector('button[data-fn="copy"]').addEventListener('click', async (evt) => {
    evt.stopPropagation();

    // Capture the button element synchronously to avoid losing it after awaits
    const copyBtnEle =
      evt.currentTarget && evt.currentTarget instanceof Element
        ? evt.currentTarget
    ...
    if (!copied) {
      ...
      if (btn && icon) {
        ...
        if (btn.dataset.copyIconTimerId) {
          try {
            clearTimeout(Number(btn.dataset.copyIconTimerId));
          } catch (_) {
            /* noop */
          }
        }
  ...
  divContainer.querySelector('button[data-fn="reload"]').addEventListener('click', async (evt) => {
    evt.stopPropagation();
    const evtTarget = libs.evtTarget(evt);
    ...
    modalEle.querySelector('button[data-fn="copy"]').addEventListener('click', async () => {
      ...
      if (ok) {
        try {
          ...
          const inst = Modal?.getInstance ? Modal.getInstance(modalEle) : Modal ? new Modal(modalEle) : null;
          inst?.hide();
        } catch (_) {
          /* noop */
        }
...
async function tts(chatID, text) {
  ...
  chatContainer.querySelector(`#${chatID} .ai-response .ai-resp-audio`)?.remove();
  chatContainer.querySelector(`#${chatID} .ai-response`).insertAdjacentElement('beforeend', audio);
...
function combineRefs(arr) {
  ...
  for (const val of arr) {
    ...
    if (val.startsWith('https') || val.startsWith('http')) {
      ...
      // sometimes refs are just plain text, not url
      // markdown += `- \`${val}\`\n`;
...
async function bindUserInputSelectFilesBtn() {
  ...
  chatContainer.querySelector('.user-input .btn.upload').addEventListener('click', async (evt) => {
    evt.stopPropagation();
    // const evtTarget = libs.evtTarget(evt);
...
async function setupChatInput() {
  ...
  // bind input press enter
  {
    ...
    chatPromptInputEle.addEventListener('compositionstart', async (evt) => {
      evt.stopPropagation();
      isComposition = true;
    });
    chatPromptInputEle.addEventListener('compositionend', async (evt) => {
      evt.stopPropagation();
      isComposition = false;
    });

    chatPromptInputEle.addEventListener('keydown', async (evt) => {
      evt.stopPropagation();
  ...
  {
    await libs.KvAddListener(
      KvKeyPrefixSessionConfig,
  ...
  // bind input button
  if (chatPromptInputBtn) {
    chatPromptInputBtn.addEventListener('click', async (evt) => {
      evt.stopPropagation();
      // Instantly restore input to default height to avoid obscuring AI response
      restoreUserInputDefaultStateAndFocus();
      await sendChat2Server();
...
async function setupChatSwitchs() {
  {
    ...
    chatContainer.querySelector('#switchChatEnableMCP').addEventListener('change', async (evt) => {
      evt.stopPropagation();
      const switchEle = libs.evtTarget(evt);
      const sconfig = await getChatSessionConfig();
    ...
    chatContainer.querySelector('#selectDrawNImage').addEventListener('change', async (evt) => {
      evt.stopPropagation();
      const switchEle = libs.evtTarget(evt);
      const sconfig = await getChatSessionConfig();
    ...
    chatContainer.querySelector('#switchChatEnableTalking').addEventListener('change', async (evt) => {
      evt.stopPropagation();
      const switchEle = libs.evtTarget(evt);
      const sconfig = await getChatSessionConfig();
...
async function updateChatVisionSelectedFileStore() {
  ...
  chatContainer.querySelectorAll('.pinned-files .bi.bi-trash').forEach((item) => {
    item.addEventListener('click', async (evt) => {
      evt.stopPropagation();
...
async function reloadAiResp(chatID, overwriteSendChat2Server) {
  ...
  chatEle.querySelector('.role-human .control .bi-trash').addEventListener('click', deleteBtnHandler);
  chatEle.querySelector('.role-human .control .bi-pencil-square').addEventListener('click', bindEditHumanInput);
...
function putBackAttachmentsInUserInput(chatID) {
  ...
  const attachEles = chatEle.querySelectorAll('.role-human .text-start img') || [];
  let attachHTML = '';
...
function bindEditHumanInput(evt) {
  ...
  newHumanEl.querySelector('.btn.save').addEventListener('click', async (evt) => {
    evt.stopPropagation();
  ...
  newHumanEl.querySelector('.btn.cancel').addEventListener('click', async (evt) => {
    evt.stopPropagation();
    ...
    oldHumanEl.querySelector('.control .bi-trash')?.addEventListener('click', deleteBtnHandler);
    oldHumanEl.querySelector('.control .bi-pencil-square')?.addEventListener('click', bindEditHumanInput);
  });
}
...
async function append2Chats(isHistory, chatData, container = null) {
  ...
  if (chatOp === 'replace') {
    ...
    chatEle.querySelector('.role-ai').outerHTML = chatEleHtml;
  }
...
async function setupMCPManager() {
  ...
  const renderMCPList = async () => {
    ...
    servers.forEach((server) => {
      ...
      const toolRows = (Array.isArray(server.tools) ? server.tools : [])
        .map((t) => {
  ...
  const bindMCPListEvents = () => {
    ...
    listContainer.querySelectorAll('.mcp-tool-enable-switch').forEach((el) => {
      el.addEventListener('change', async (e) => {
        ...
        const syncedNames = (Array.isArray(server.tools) ? server.tools : []).map((t) => extractToolNameFromDefinition(t)).filter(Boolean);
  ...
  (async () => {
    try {
      ...
      const results = await Promise.all(toFetch.map((server) => syncMCPServerTools(server.id, { silent: true, showSpinner: false })));
...
async function setupConfig() {
  ...
  {
    const apitokenInput = configContainer.querySelector('.input.api-token');
  ...
  // bind api_base
  {
    const apibaseInput = configContainer.querySelector('.input.api-base');
  ...
  {
    const maxtokenInput = configContainer.querySelector('.input.contexts');
  ...
  {
    const maxtokenInput = configContainer.querySelector('.input.max-token');
    maxtokenInput.addEventListener('input', async (evt) => {
      evt.stopPropagation();
  ...
  {
    const maxtokenInput = configContainer.querySelector('.input.temperature');
    maxtokenInput.addEventListener('input', async (evt) => {
      evt.stopPropagation();
  ...
  {
    const maxtokenInput = configContainer.querySelector('.input.presence_penalty');
    maxtokenInput.addEventListener('input', async (evt) => {
      evt.stopPropagation();
  ...
  {
    const maxtokenInput = configContainer.querySelector('.input.frequency_penalty');
    maxtokenInput.addEventListener('input', async (evt) => {
      evt.stopPropagation();
  ...
  {
    const staticConfigInput = configContainer.querySelector('.system-prompt .input');
  ...
  // bind reset button
  {
    configContainer.querySelector('.btn.reset').addEventListener('click', async (evt) => {
      evt.stopPropagation();
  ...
  {
    document.querySelectorAll('.btn.clear-chats').forEach((ele) => {
  ...
  // bind submit button
  {
    configContainer.querySelector('.btn.submit').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      location.reload();
    });
  }
  ...
  // bind sync key
  {
    ...
    syncKeyEle.addEventListener('input', async (evt) => {
      evt.stopPropagation();
  ...
  {
    configContainer.querySelector('.btn[data-app-fn="upload-config"]').addEventListener('click', async (evt) => {
      try {
        ShowSpinner();
    ...
    configContainer.querySelector('.btn[data-app-fn="download-config"]').addEventListener('click', async (evt) => {
      try {
        ShowSpinner();
...
async function EditFavSystemPromptHandler(evt) {
  ...
  const titleInput = saveSystemPromptModelEle.querySelector('.modal-body input.title');
  titleInput.value = shortcut.title;

  const descriptionInput = saveSystemPromptModelEle.querySelector('.modal-body textarea.user-input');
...
async function appendPromptShortcut(shortcut, storage = false) {
  ...
  ele.querySelector('i.bi-pencil-square').addEventListener('click', EditFavSystemPromptHandler);

  // add delete click event
  ele.querySelector('i.bi-trash').addEventListener('click', async (evt) => {
    evt.stopPropagation();
...
async function setupPromptManager() {
  // restore shortcuts from kv
  {
    ...
    configContainer.querySelector('.prompt-shortcuts .badge').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      const promptInput = configContainer.querySelector('.system-prompt .input');
  ...
  const saveSystemPromptModal = new window.bootstrap.Modal(saveSystemPromptModelEle);
  {
    configContainer.querySelector('.system-prompt .bi.save-prompt').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      const promptInput = configContainer.querySelector('.system-prompt .input');

      saveSystemPromptModelEle.querySelector('.modal-body textarea.user-input').innerHTML = promptInput.value;
  ...
  {
    configContainer.querySelector('.system-prompt .bi.open-prompt-market').addEventListener('click', async (evt) => {
      evt.stopPropagation();
  ...
  {
    promptMarketModal.querySelector('.modal-body .save').addEventListener('click', async (evt) => {
      evt.stopPropagation();
...
async function setupPrivateDataset() {
  ...
  // bind header's custom qa button
  {
    ...
    document.querySelector('#headerbar .qa-models a[data-model="qa-custom"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      pdfFileModal.show();
    });
  }
  ...
  // bind datakey to kv
  {
    const datakeyEle = pdfchatModalEle.querySelector('div[data-field="data-key"] input');
    ...
    datakeyEle.addEventListener('change', async (evt) => {
      evt.stopPropagation();
  ...
  // bind file upload
  {
    ...
    pdfchatModalEle.querySelector('div[data-field="pdffile"] input').addEventListener('change', async (evt) => {
      evt.stopPropagation();
      ...
      if (['.pdf', '.md', '.ppt', '.pptx', '.doc', '.docx'].indexOf(fileext) === -1) {
        // remove choosen
        pdfchatModalEle.querySelector('div[data-field="pdffile"] input').value = '';
      ...
      pdfchatModalEle.querySelector('div[data-field="dataset-name"] input').value = filename;
    });

    // bind upload button
    pdfchatModalEle.querySelector('div[data-field="buttons"] button[data-fn="upload"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();

      if (pdfchatModalEle.querySelector('div[data-field="pdffile"] input').files.length === 0) {
      ...
      form.append('file', pdfchatModalEle.querySelector('div[data-field="pdffile"] input').files[0]);
      form.append('file_key', pdfchatModalEle.querySelector('div[data-field="dataset-name"] input').value);
      form.append('data_key', pdfchatModalEle.querySelector('div[data-field="data-key"] input').value);
  ...
  const bindDatasetDeleteBtn = () => {
    const datasets = pdfchatModalEle.querySelectorAll('div[data-field="dataset"] .dataset-item .bi-trash');
  ...
  // bind list datasets
  {
    pdfchatModalEle.querySelector('div[data-field="buttons"] button[data-fn="refresh"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      ...
      const datasetListEle = pdfchatModalEle.querySelector('div[data-field="dataset"]');
      let datasetsHTML = '';
      ...
      body.selected.forEach((dataset) => {
        datasetListEle.querySelector(`div[data-filename="${dataset}"] input[type="checkbox"]`).checked = true;
      });
  ...
  // bind list chatbots
  {
    pdfchatModalEle.querySelector('div[data-field="buttons"] a[data-fn="list-bot"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      ...
      datasetListEle.querySelectorAll('div[data-field="dataset"] .chatbot-item input[type="checkbox"]').forEach((ele) => {
  ...
  // bind share chatbots
  {
    pdfchatModalEle.querySelector('div[data-field="buttons"] a[data-fn="share-bot"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      const evtTarget = libs.evtTarget(evt);

      new window.bootstrap.Dropdown(evtTarget.closest('.dropdown')).hide();

      const checkedChatbotEle = pdfchatModalEle.querySelector('div[data-field="dataset"] .chatbot-item input[type="checkbox"]:checked');
  ...
  {
    pdfchatModalEle.querySelector('div[data-field="buttons"] a[data-fn="build-bot"]').addEventListener('click', async (evt) => {
      evt.stopPropagation();
      const evtTarget = libs.evtTarget(evt);
      ...
      pdfchatModalEle.querySelectorAll('div[data-field="dataset"] .dataset-item input[type="checkbox"]').forEach((ele) => {
        if (ele.checked) {
          selectedDatasets.push(ele.closest('.dataset-item').getAttribute('data-filename'));
        }
      });
        ...
        try {
          ...
          const resp = await fetch('/ramjet/gptchat/ctx/build', {
            ...
            body: JSON.stringify({
              ...
              data_key: pdfchatModalEle.querySelector('div[data-field="data-key"] input').value,
            }),
          });
